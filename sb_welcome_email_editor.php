...
Description: FORKED: Added wpMandrill compatibility. Allows you to change the content, layout and even add an attachment for many of the inbuilt Wordpress emails. Simple!
Version: 400.8
Author: Sean Barton
	...
	function wp_new_user_notification($user_id, $depracated='', $notify = '') {
		...
		if ($user = new WP_User($user_id)) {
			...
			if ($settings->admin_notify_user_id) {
				...
				foreach ($admins as $admin_id) {
					if ($admin = new WP_User($admin_id)) {
						wpMandrill::mail($admin->user_email, $admin_subject, $admin_message, $headers);
					}
				}
			}
			...
			if ($notify && $notify != 'admin') { //needs to be like this for backwards compatibility
				...
				wpMandrill::mail($user_email, $user_subject, $user_message);
			}
		}
